<template>
  <div class="imagesComponentKea">
    ...
    <div class="prev" @click.stop="prevImg">
      <img src="../../assets/images/customMadeImgs/bar/next.png" alt="" srcset="">
    </div>
    <div class="next" @click.stop="nextImg" v-show="index+1<this.$store.state.guaranteeImg.length">
      <img src="../../assets/images/customMadeImgs/bar/next.png" alt="" srcset="">
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    prevImg(){
      if(this.index >= 1){
        this.index--;
        this.$set(this.imgSrc, 0, this.$store.state.guaranteeImg[this.index]);
      }else{
          this.$router.push({name:'KhjlzxVideoKea',query: {
    ...
    // 下一页方法
    nextImg(){
      if(this.index < this.$store.state.guaranteeImg.length - 1){
        this.index++;
        this.$set(this.imgSrc, 0, this.$store.state.guaranteeImg[this.index]);
      }else{
        return false
